...
{ // Функція для обрахунку суми масиву
	int sum = 0;
...
void randgen(long min, long max, int number[SIZE])
{
	int i = 0;
	for (i = 0; i < SIZE; i++)
	{													   // цикл для внесення в масив рандомних значень
...
int main(int argc, char *argv[])
{
	...
	long max = 100;
	if (argc > 3)
	{ // перевірка кількості аргументів в разі якого буде помилка бо забагато
		printf("помилка занадто багато аргументів");
		return 1;
	}
	else if (argc == 3)
	{									 // перевірка кількості аргументів яка нам підходить з кодом всередині для присваювання значень в min та max
		min = strtol(argv[1], NULL, 10); // strtol- функція для переведення string to long як альтернатива atoi
		max = strtol(argv[2], NULL, 10);
		if (min > max)
		{ // перевірка якщо перший аргумент більше ніж другий вносимо то в той
			long temp = min;
			min = max;
			max = temp;
		}
	...
	{ // перевірка для 2 аргументів з кодом внесення якщо аргумент більше ніж максимальне значення вносимо його в мінімальне , а якщо ні то в максимальне
